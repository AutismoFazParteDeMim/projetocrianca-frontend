...
function Welcome({ navigation }) {
    ...
    return (
        <View style={styles(colors).container}>
            <View style={styles(colors).topSide}>
                <Text style={[styles(colors).globalTitle, styles(colors).text, styles(colors).title]}>Seja Bem Vindo(a)!</Text>
                <Text style={[styles(colors).globalText, styles(colors).text]}>Lorem ipsum nunc primiskj suscipit dictumst, sociosqu vehicula morbi fermentumf litora, facilisis tristiquedwgj  consectetur cubilia.</Text>
            </View>

            <View style={styles(colors).bottomSide}>
            ...
            <CustomModal visible={showModal} title="Cadastro" closeAction={() => setShowModal(false)}>
                <ScrollView>
                    <Text style={styles(colors).globalText}>
